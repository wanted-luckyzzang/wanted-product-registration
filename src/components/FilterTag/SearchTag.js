...
const SearchTag = ({ searchWord, setSearchWord, selectedTag, setSelectedTag, inputRef }) => {
  ...
  const addTag = useCallback(
    (event) => {
      ...
      if (!selectedTag.includes(newTag)) setSelectedTag([...selectedTag, newTag]);
  ...
  return (
    <Grid isFlex isWrap scrollY height='7rem' position='absolute' top='3.5rem' border padding='.5rem'>
      {searchTagList.sort().map((tag, idx) => (
        <Grid width='6.3rem' isFlex justify='center' margin='0 0 0 0.3rem' key={idx}>
          <Button height='2rem' bg={bg.tag} _onClick={addTag} border='' margin='.1rem 0'>
            {tag}
          </Button>
        </Grid>
      ))}
